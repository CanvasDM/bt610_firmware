...
static const struct bt_data testAD[] = {
	BT_DATA_BYTES(BT_DATA_FLAGS, (BT_LE_AD_GENERAL | BT_LE_AD_NO_BREDR)),
	BT_DATA_BYTES(BT_DATA_MANUFACTURER_DATA, 0x0d, 0x18, 0x0f, 0x18, 0x0a,
		      0x18, 0x99, 0x98, 0x97, 0x96, 0x95, 0x94, 0x93, 0x92,
		      0x91, 0x90, 0x89, 0x88, 0x87, 0x86, 0x85, 0x84),
		      //0x83), // 0x82),
};
	...
	if (advertising == true) {
		...
		if (r == 0) {
			if (extendPhyEnbled == true) {
				r = bt_le_ext_adv_set_data(extendedAdv, testAD,
							   ARRAY_SIZE(testAD),
							   NULL, 0);
			} else {
...
void CreateAdvertisingExtendedParm(void)
{
	...
	err = bt_le_ext_adv_set_data(extendedAdv, testAD, ARRAY_SIZE(testAD),
				     NULL, 0);
	if (err) {
